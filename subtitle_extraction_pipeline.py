...
def subtitle_prompt(times: list[float]) -> str:
    ...
    return f"""你是视频字幕取证助手。你的唯一任务是识别画面中的对白、旁白或翻译字幕，不要做导演拉片，不要描述画面，不要总结剧情，也不要根据声音猜台词。

这些图片按发送顺序对应视频时间：{mapping}。

识别范围：每张图片只检查画面高度下半部分，重点是底部或下方居中的字幕区域。画面上半部分和中部的文字全部忽略，即使它看起来像对白也不要抄录。台标、水印、按钮、聊天框、海报、书本、路牌、包装、衣服、片名、标题卡、场景文字和其他非字幕文字全部忽略。

任务：找出下半部分真正作为对白、旁白或翻译字幕出现的可读文字，用于后续重新配音。字幕可能有两行，请合并为一条文字并保留自然句读；不要添加【】、[]、横杠、斜杠、竖线、项目符号、Markdown 或其他分隔符。看不清、被遮挡、位置不在下半部分或无法确认时，必须返回 visible=false、text=""，不要猜测、不要根据相邻帧或声音补全。
